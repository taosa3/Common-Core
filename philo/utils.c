...
int parse_args(int ac, char **av, t_table *table)
{
    if (ac != 5 && ac != 6)
    {
        fprintf(stderr, "Usage: %s number_of_philosophers time_to_die time_to_eat time_to_sleep [number_of_times_each_philo_must_eat]\n", av[0]);
        return (1);
    }
    ...
    if (table->number_of_philosophers <= 0 || table->time_to_die < 0 || table->time_to_eat < 0 || table->time_to_sleep < 0)
    {
